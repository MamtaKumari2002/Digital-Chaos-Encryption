...
class MainApplication:
    def __init__(self, app, key):
        self.app = app
        self.app.geometry('%sx%s' % (
            int(self.app.winfo_screenwidth()/3), int(app.winfo_screenheight()/2)))
        ...
        self.mode_label = tk.Label(
            self.app, text="Select mode:", font=('Helvetica', 9))
        self.mode_radio_e = tk.Radiobutton(
            self.app, text="Encrypt", variable=self.selection_value, value=1, command=self.selection)
        self.mode_radio_d = tk.Radiobutton(
            self.app, text="Decrypt", variable=self.selection_value, value=2, command=self.selection)

        # Choose
        self.choose_label = tk.Label(
            self.app, textvariable=self.choose_label_text, font=('Helvetica', 10))
        self.choose_entry = tk.Text(self.app, width=0, height=0)
        self.choose_entry.config(state=tk.DISABLED)
        self.add_button = tk.Button(
            self.app, text="Add", command=self.chooseImg)
        self.clear_button = tk.Button(self.app, text="Clear", command=lambda: [self.choose_entry.config(
            state=tk.NORMAL), self.choose_entry.delete(1.0, tk.END), self.choose_entry.config(state=tk.DISABLED), self.listFilenames.clear()])

        # Output
        self.output_label = tk.Label(
            self.app, text="Choose output destination", font=('Helvetica', 10))
        self.output_entry = tk.Entry(self.app)
        self.output_entry.config(state=tk.DISABLED)
        self.output_button = tk.Button(
            self.app, text="Select folder", command=self.chooseDestination)

        # Key
        self.private_key_label = tk.Label(
            self.app, text="Your private key: ", font=('Helvetica', 10))
        ...
        self.public_key_label = tk.Label(
            self.app, textvariable=self.key_label_text, font=('Helvetica', 10))
        self.public_key_entry = tk.Entry(app, width=0)

        # Progress
        self.progressText = tk.StringVar()
        self.progress_label = tk.Label(
            self.app, textvariable=self.progressText, font=('Helvetica', 9))

        # Encrypt
        self.encrypt_button = tk.Button(self.app, textvariable=self.encrypt_text, command=lambda: thread._start_new_thread(
            self.beginEncryption, (self.listFilenames, key, self.public_key_entry.get())))

        # Grid
        self.app.grid_columnconfigure(2, weight=1)
        self.app.grid_rowconfigure(3, weight=1)

        self.mode_label.grid(row=0, column=0, padx=(8, 0),
                             pady=(10, 5), sticky='wn')
        self.mode_radio_e.grid(row=0, column=1, padx=(
            4, 0), pady=(10, 0), sticky='wn')
        self.mode_radio_d.grid(row=0, column=2, padx=(
            4, 0), pady=(10, 0), sticky='wn')

        self.choose_label.grid(row=1, column=0, padx=(
            8, 0), pady=(10, 5), sticky='wn', columnspan=3)
        self.choose_entry.grid(row=2, column=0, sticky='wens', padx=(
            10, 5), rowspan=3, columnspan=3)
        self.add_button.grid(row=2, column=3, sticky='wen', padx=(5, 10))
        self.clear_button.grid(
            row=3, column=3, sticky='wes', padx=(5, 10), pady=(5, 0))

        self.output_label.grid(row=5, column=0, padx=(
            8, 0), pady=(10, 5), sticky='wn', columnspan=3)
        self.output_entry.grid(
            row=6, column=0, sticky='wen', padx=(10, 5), columnspan=3)
        self.output_button.grid(row=6, column=3, sticky='wen', padx=(5, 10))

        self.private_key_label.grid(row=7, column=0, padx=(
            8, 0), pady=(5, 5), sticky='wn', columnspan=3)
        self.private_key_entry.grid(
            row=8, column=0, sticky='wen', padx=10, columnspan=4)
        self.public_key_label.grid(row=9, column=0, padx=(
            8, 0), pady=(5, 5), sticky='wn', columnspan=3)
        self.public_key_entry.grid(
            row=10, column=0, sticky='wen', padx=10, columnspan=4)

        self.progress_label.grid(row=11, column=0, sticky='swn', padx=(
            7, 0), pady=(20, 10), columnspan=3)

        self.encrypt_button.grid(
            row=11, column=3, sticky='senw', padx=(0, 10), pady=(20, 10))
    ...
    def beginEncryption(self, listFilenames, private_key, public_key):
        ...
        self.progressText.set(self.encrypt_text.get(
        )+"ion finished. ("+str(len(listFilenames))+"/"+str(len(listFilenames))+")")
        self.clearAllText()
        self.changeButtonState(tk.NORMAL)

    def showBar(self, filename, index, length):
        self.progressText.set(self.encrypt_text.get(
        )+"ing "+os.path.basename(filename)+" ("+str(index)+"/"+str(length)+")")
...
class LoginApp:
    def __init__(self, app):
        ...
        self.login_label = tk.Label(
            self.frame, text="Login with private key", font=('Helvetica', 10, 'bold'))
        self.login_entry = tk.Entry(self.frame)
        self.login_button = tk.Button(self.frame, text="Login", font=(
            'Helvetica', 10), command=lambda: self.start_intro(self.login_entry.get()))
        self.generate_button = tk.Button(self.frame, text="Generate new key pairs", font=(
            'Helvetica', 10), command=self.generateKeyPairs)
    ...
    def generateKeyPairs(self):
        ...
        f = fd.asksaveasfile(title="Select directory", initialfile='keypairs',
